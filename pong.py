...
pen2.goto(0, 230)
pen2.write(
    "You are Player A: Use W / S to move the paddle up / down",
    align="center",
...
while True:
    ...
    if ball.xcor() > ((WIDTH / 2) - (2 * paddle_width)):
        ...
        pen1.clear()
        pen1.write(
            "Player A: {}    AI: {}".format(score_A, score_B),
            align="center",
            font=("Courier", 24, "bold"),
        )
    ...
    # Collisons
    if (
        (ball.xcor() > (WIDTH / 2 - (clearance + (2 * paddle_width))))
        and (ball.xcor() < (WIDTH / 2 - clearance))
        and (ball.ycor() < (paddle_b.ycor() + clearance))
        and (ball.ycor() > (paddle_b.ycor() - clearance))
    ):
    ...
    if (
        (ball.xcor() < -1 * (WIDTH / 2 - (clearance + (2 * paddle_width))))
        and (ball.xcor() > -1 * (WIDTH / 2 - clearance))
        and (ball.ycor() < (paddle_a.ycor() + clearance))
        and (ball.ycor() > (paddle_a.ycor() - clearance))
    ):
